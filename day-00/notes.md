...
_Note: This problem took me 5 hours to solve._

Today's problem took me surprisingly long to solve, mainly due to me wanting to implement the solution in a specific way (`.dfs(func)`). I also made many changes to how I structured my code.

Firstly, I changed the structure of the code from two separate `solve_part_X()` functions into a single `Solver::solve()` function. This allows me to reuse code between the parts, such as the part parsing the input.

I then implemented a `TreeNode` structure, which contains the data fields as well as a vector of children nodes. The naive way to implement this would be
...
However, Googled and realized it is not possible due to lifetime issues... Fixed `.dfs(&closure)` infinite recursion in compilation phase error by passing in the closure as a `&dyn`.
